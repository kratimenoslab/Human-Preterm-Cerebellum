library(here)
setwd(here("enrichment_analysis/project_cogaps/bedfiles/"))

gene.anno.file <- here("ref/gene_meta_hg38.txt") 
gene.meta <- read.table(gene.anno.file,sep="\t",header=T)

cell <- read.csv(here("enrichment_analysis/project_cogaps/score/cogaps_score.csv"))
...
for(i in 1:length(modules)){
	...
	bed <- gene.meta[is.element(gene.meta$Gene.name,genes),2:4]
