...
data_sub <- data_ptsub[data_ptsub['Cell_Type'] == 'Granular',] # SUBSET TO GCs

for (gene in colnames(log_spatial_matrix)) { # Loop through each gene
    gene_expression_single <- data_sub[, gene] # Extracting gene expression for a single gene
...
write.csv(beta_estimates, paste0(savename, '_Granular.csv'))
